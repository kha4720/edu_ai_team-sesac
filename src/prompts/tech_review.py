...
TECH_REVIEW_GATE3_INSTRUCTION = """다음 구현 명세서 4종 (Data Schema / State Machine / Prompt Spec / Interface Spec) 을
**기술 실행 가능성** 관점에서 검토하라.

후속 단계는 이 4종을 직접 근거로 코드를 작성하는 단계다.
즉 4종이 "구현자가 그대로 코드로 옮길 수 있는 수준"이어야 한다.

## 검토 항목

### 1) 구현 가능성 (implementation_buildability)
- 구현 명세서 4종이 **실제 구현 가능한 수준** 인가?
- Data Schema 의 필드 타입·enum 이 표준 데이터 타입으로 매핑 가능한가?
- State Machine 의 상태 전이 규칙이 모호하지 않고 코드로 그대로 구현 가능한가?
- Prompt Spec 의 모드별 분기가 LLM 호출 분기 로직으로 변환 가능한가?
- Interface Spec 의 API 스키마가 실제 HTTP 요청/응답 형태로 구현 가능한가?

### 2) MVP 제약 적합성 (mvp_constraint_fit)
- 헌법(Constitution) 에 박힌 MVP 제약 (단순 프론트 + LLM API 수준) 안에서 4종이 구현 가능한 구조인가?
- 4종 어딘가에 MVP 범위를 벗어나는 기술 요구(다중 백엔드 / 비동기 큐 / DB 마이그레이션 / 외부 OAuth 등) 가 등장하지 않는가?
- 모듈 간 결합도가 단기 구현에 부담되는 수준은 아닌가?
...
```
{
  "implementation_buildability": {"ok": true|false, "issue": "..."},
  "mvp_constraint_fit":          {"ok": true|false, "issue": "..."},
  "summary": "..."
}
```
